...
''')
cursor.execute('''
CREATE TABLE IF NOT EXISTS sub_categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, category_id INTEGER NOT NULL, FOREIGN KEY (category_id) REFERENCES categories (id))
''')
cursor.execute('''
...
''')
print("Таблица 'products' создана (без цены и количества).")

cursor.execute('''
CREATE TABLE IF NOT EXISTS sizes (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE)
''')
cursor.execute('''
CREATE TABLE IF NOT EXISTS colors (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE)
''')
print("Таблицы-справочники 'sizes' и 'colors' созданы.")
...
connection.commit()
connection.close()
print(f"\n✅ База данных '{DB_FILE}' успешно пересоздана с новой структурой для вариантов товаров!")